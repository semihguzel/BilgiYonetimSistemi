   ...
   public class BolumlerDerslerMapping : EntityTypeConfiguration<BolumlerDersler>
    {
        public BolumlerDerslerMapping()
        {
            ...
            HasRequired(x => x.BolumunDersi).WithMany(x => x.DersinBolumleri).HasForeignKey(x => x.BolumID); 
            HasRequired(x => x.DersinBolumu).WithMany(x => x.BolumDersleri).HasForeignKey(x => x.DersID); 
