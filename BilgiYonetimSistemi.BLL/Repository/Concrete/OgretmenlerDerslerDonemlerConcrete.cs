...
namespace BilgiYonetimSistemi.BLL.Repository.Concrete
{
    public class OgretmenlerDerslerDonemlerConcrete : IOgretmenlerDerslerDonemler
    {
        ...
        {
            return _ogretmenlerDerslerDonemlerRepository.GetEntity().Where(x => x.OgretmenID == ogretmenId).Select(x => new DersDTO { DersAdi = x.OgretmeninDersi.DersAdi, DersID = x.DersID, DonemID = x.DonemID,DonemYili = x.OgretmenDersinDonemi.DonemYili }).ToList();
        }
    }
}
